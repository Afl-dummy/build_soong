...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/afterlife-priv/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["AfterlifeDesc"] = config["BuildDesc"]
  config["AfterlifeDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] != "eng":
    ...
    print(f"ro.build.display.id?={config['AfterlifeDesc']}")
  ...
  print(f"ro.afterlife.device={config['AfterlifeDevice']}")
